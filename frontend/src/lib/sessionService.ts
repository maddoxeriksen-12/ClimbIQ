    ...
    if (input.pre_session_data && session) {
      ...
      try {
        await supabase.from('pre_session_data').insert({
          ...
          session_environment: pre.session_environment as string | null,
          planned_duration: pre.planned_duration as number | null,
          partner_status: pre.partner_status as string | null,
          crowdedness: pre.crowdedness as number | null,
          // B. Systemic Recovery & Lifestyle
          sleep_quality: pre.sleep_quality as number | null,
          sleep_hours: pre.sleep_hours as number | null,
          stress_level: pre.stress_level as number | null,
          fueling_status: pre.fueling_status as string | null,
          hydration_feel: pre.hydration_feel as string | null,
          skin_condition: pre.skin_condition as string | null,
          finger_tendon_health: pre.finger_tendon_health as number | null,
          doms_locations: pre.doms_locations as string[] | null,
          doms_severity: pre.doms_severity as number | null,
          menstrual_phase: pre.menstrual_phase as string | null,
          // C. Intent & Psych
          motivation: pre.motivation as number | null,
          primary_goal: pre.primary_goal as string | null,
          // D. Physical Readiness
          warmup_rpe: pre.warmup_rpe as string | null,
          warmup_compliance: pre.warmup_compliance as string | null,
          upper_body_power: pre.upper_body_power as number | null,
          shoulder_integrity: pre.shoulder_integrity as number | null,
          leg_springiness: pre.leg_springiness as number | null,
          finger_strength: pre.finger_strength as number | null,
        })
      } catch (preErr) {
    ...
    if (input.post_session_data) {
      ...
      try {
        await supabase.from('post_session_data').insert({
          ...
          hardest_grade_sent: post.hardest_grade_sent as string | null,
          hardest_grade_attempted: post.hardest_grade_attempted as string | null,
          volume_estimation: post.volume_estimation as string | null,
          strength_metrics: (post.strength_metrics ?? []) as any,
          dominant_style: post.dominant_style as string | null,
          // B. Subjective Experience
          rpe: post.rpe as number | null,
          session_density: post.session_density as string | null,
          intra_session_fueling: post.intra_session_fueling as string | null,
          // C. Failure Analysis
          limiting_factors: post.limiting_factors as string[] | null,
          flash_pump: post.flash_pump as boolean | null,
          // D. Health & Injury Update
          new_pain_location: post.new_pain_location as string | null,
          new_pain_severity: post.new_pain_severity as number | null,
          fingers_stiffer_than_usual: post.fingers_stiffer_than_usual as boolean | null,
          skin_status_post: post.skin_status_post as string | null,
          doms_severity_post: post.doms_severity_post as number | null,
          finger_power_post: post.finger_power_post as number | null,
          shoulder_mobility_post: post.shoulder_mobility_post as number | null,
          // E. Learning Loop
          prediction_error: post.prediction_error as number | null,
        })
      } catch (postErr) {
