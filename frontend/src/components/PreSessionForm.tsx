...
import { hasDraft, loadDraft, saveDraft, clearDraft, getDraftAge } from '../lib/formStorage'
...
export function PreSessionForm({ onComplete }: PreSessionFormProps) {
  ...
  useEffect(() => {    // Check for draft on mount
    if (hasDraft()) {
    ...
    const interval = setInterval(() => {
      if (formData.session_environment || formData.primary_goal) {
        saveDraft(formData as unknown as Record<string, unknown>)
  ...
  const handleLoadDraft = () => {
    ...
    if (draft?.data) {
      setFormData(draft.data as unknown as PreSessionData)
  ...
  const sectionAComplete = formData.session_environment && formData.partner_status
  const sectionBComplete = formData.fueling_status && formData.hydration_feel && formData.skin_condition
  const sectionCComplete = formData.primary_goal

  // Scroll to warmup section
  const scrollToWarmup = () => {
    warmupRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  }
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      {showDraftPrompt && (
        <div className="mb-6 rounded-2xl border border-blue-500/30 bg-blue-500/10 p-5 animate-in fade-in slide-in-from-top-4 duration-500">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-xl">💾</span>
                <h3 className="font-semibold text-blue-300">Resume Previous Session?</h3>
              </div>
              <p className="text-sm text-slate-300 mb-3">
                We found a saved draft from {getDraftAge()}. Would you like to continue where you left off?
              </p>
              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={handleLoadDraft}
                  className="px-4 py-2 rounded-xl bg-blue-600 text-white text-sm font-medium hover:bg-blue-500 transition-all"
                >
                  Load Draft
                </button>
                <button
                  ...
                  className="px-4 py-2 rounded-xl border border-white/10 text-slate-300 text-sm font-medium hover:bg-white/5 transition-all"
                >
                  Start Fresh
                </button>
              </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="mb-2 flex items-center gap-2">
          ...
          {sectionAComplete && <span className="text-emerald-400 text-xs">✅ Complete</span>}
        </div>
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-4">
          ...
          <div className="flex rounded-lg bg-white/5 p-0.5 mb-3">
            <button
              ...
              className={`flex-1 py-2 px-3 rounded-md text-xs font-semibold transition-all ${isIndoor
          ...
          <div className="grid grid-cols-1 gap-1.5">
            {sessionEnvironments.map((env) => (
              <button
                ...
                className={`py-2 px-3 rounded-lg text-left text-xs font-medium transition-all ${formData.session_environment === env.value
                  ? isIndoor
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {partnerOptions.map((opt) => (
              <button
                ...
                className={`py-2 px-3 rounded-lg text-xs font-medium transition-all ${formData.partner_status === opt.value
                  ? 'bg-gradient-to-r from-fuchsia-500/20 to-cyan-500/20 text-white border border-fuchsia-500/30'
                  : 'bg-white/5 text-slate-300 border border-white/10 hover:bg-white/10'
                  }`}
              >
        ...
        <div className="mt-6 mb-2 flex items-center gap-2">
          ...
          {sectionBComplete && <span className="text-emerald-400 text-xs">✅ Complete</span>}
        </div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {fuelingOptions.map((opt) => (
              <button
                ...
                className={`py-2 px-3 rounded-lg text-xs font-medium transition-all ${formData.fueling_status === opt.value
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {hydrationOptions.map((opt) => (
              <button
                ...
                className={`py-2 px-2 rounded-lg text-xs font-medium transition-all ${formData.hydration_feel === opt.value
                  ? 'bg-gradient-to-r from-cyan-500/20 to-emerald-500/20 text-white border border-cyan-500/30'
                  : 'bg-white/5 text-slate-300 border border-white/10 hover:bg-white/10'
                  }`}
              >
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {skinOptions.map((opt) => (
              <button
                ...
                className={`py-2 px-2 rounded-lg text-xs font-medium transition-all ${formData.skin_condition === opt.value
                  ? 'bg-gradient-to-r from-cyan-500/20 to-emerald-500/20 text-white border border-cyan-500/30'
                  : 'bg-white/5 text-slate-300 border border-white/10 hover:bg-white/10'
                  }`}
              >
        ...
        <div className={`rounded-xl border backdrop-blur-sm p-4 ${showFingerWarning
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className={`w-full h-2 rounded-full bg-white/10 appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:shadow-lg ${showFingerWarning
            ...
            <div className="flex flex-wrap gap-1.5">
              {domsLocations.map((loc) => (
                <button
                  ...
                  className={`py-1.5 px-2.5 rounded-lg text-xs font-medium transition-all ${(loc.value === 'none' && formData.doms_locations.length === 0) ||
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {menstrualOptions.map((opt) => (
                <button
                  ...
                  className={`py-2 px-2 rounded-lg text-xs font-medium transition-all ${formData.menstrual_phase === opt.value
        ...
        <div className="mt-6 mb-2 flex items-center gap-2">
          ...
          {sectionCComplete && <span className="text-emerald-400 text-xs">✅ Complete</span>}
        </div>
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm p-4">
          ...
          <div className="grid grid-cols-1 gap-1.5">
            {primaryGoalOptions.map((opt) => (
              <button
                ...
                className={`py-2 px-3 rounded-lg text-left text-xs font-medium transition-all ${formData.primary_goal === opt.value
          ...
          {formData.primary_goal === 'tell_me' && (
            <div className="mt-3 p-4 rounded-xl bg-gradient-to-r from-violet-500/10 to-indigo-500/10 border border-violet-500/30 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="flex items-start gap-3">
                <span className="text-2xl">🤖</span>
                <div>
                  <p className="text-sm font-semibold text-violet-300 mb-1">✨ AI Mode Activated</p>
                  <p className="text-xs text-slate-300 leading-relaxed">
                    Your coach will analyze your readiness state and recommend the optimal session type.
                    We'll suggest a specific goal after reviewing your physical and mental state.
                  </p>
                </div>
              </div>
            </div>
        ...
        {/* Continue to Warm-Up Button */}
        {sectionAComplete && sectionCComplete && !warmupComplete && (
          <div className="mt-6 p-5 rounded-2xl border border-amber-500/30 bg-gradient-to-r from-amber-500/5 to-orange-500/5 animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="flex flex-col items-center text-center gap-3">
              <div className="flex items-center gap-2">
                <span className="text-2xl">🔥</span>
                <h3 className="font-semibold text-amber-300">Ready for Your Personalized Warm-Up?</h3>
              </div>
              <p className="text-sm text-slate-300 max-w-md">
                Based on your inputs, we've generated a custom warm-up routine tailored to your current state and goals.
              </p>
              <button
                type="button"
                onClick={scrollToWarmup}
                className="px-6 py-3 rounded-xl bg-gradient-to-r from-amber-600 to-orange-600 text-white font-semibold text-sm shadow-lg shadow-amber-500/25 hover:shadow-amber-500/40 transition-all flex items-center gap-2 hover:scale-105"
              >
                Continue to Warm-Up
        ...
        {/* ============================================ */}
        <div className="mt-6 mb-2" ref={warmupRef}>
                ...
                <div className="grid grid-cols-1 gap-1.5">
                  {warmupRpeOptions.map((opt) => (
                    <button
                      ...
                      className={`py-2 px-3 rounded-lg text-left text-xs font-medium transition-all ${formData.warmup_rpe === opt.value
                ...
                <div className="grid grid-cols-1 gap-1.5">
                  {warmupComplianceOptions.map((opt) => (
                    <button
                      ...
                      className={`py-2 px-3 rounded-lg text-left text-xs font-medium transition-all ${formData.warmup_compliance === opt.value
                        ? 'bg-gradient-to-r from-amber-500/20 to-orange-500/20 text-white border border-amber-500/30'
                        : 'bg-white/5 text-slate-300 border border-white/10 hover:bg-white/10'
                        }`}
                    >
