...
import { CorrelationWidgetCompact } from '../components/CorrelationWidget'
import { GoalWidget } from '../components/GoalWidget'
import { LiveClimbTracker } from '../components/LiveClimbTracker'
import { CoachActionCenter } from '../components/CoachActionCenter'
import { GradeProgressionChart } from '../components/GradeProgressionChart'
import { getActiveSession, type ActiveSessionData } from '../lib/sessionStorage'
...
export function Dashboard() {
  ...
  const refreshData = useCallback(async () => {
    ...
    // Check for active session synchronously first to avoid flash
    ...
    try {
      const [statsResult, sessionsResult] = await Promise.all([
        getSessionStats(),
        getRecentSessions(10), // Increased to 10 for better chart data
      ])
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      {/* 1. Hero / Action Center (Replaces static header) */}
      {!activeSession ? (
        <CoachActionCenter
          userName={firstName}
          lastSessionDate={recentSessions[0]?.started_at}
          onSessionStart={() => setActiveSession(getActiveSession())}
        />
      ) : (
        <div className="rounded-2xl border border-emerald-500/30 bg-gradient-to-r from-emerald-500/10 to-cyan-500/10 p-6 animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        {displayStats.map((stat) => {
          const content = (
            <>
              ...
              <p className={`text-xs mt-2 ${stat.trend === 'up' ? 'text-emerald-400' :
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-semibold text-lg flex items-center gap-2">
                <span className="text-xl">📈</span> Grade Progression
              </h2>
              <Link to="/stats" className="text-sm text-fuchsia-400 hover:text-fuchsia-300">
                Full Analytics →
              </Link>
            </div>
            <GradeProgressionChart sessions={recentSessions} />
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-5">
            <div className="flex items-center gap-2 mb-4">
              <h2 className="font-semibold">Coach's Insights</h2>
